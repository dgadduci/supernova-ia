...
Persist the pre-commerce customer routing context for shared WhatsApp
destinations, expose manual commerce selection and explicit commerce switching
on top of it, and never invoke the order pipeline or silently change the
selected commerce. Selection is authoritative only after explicit confirmation;
the original pending text is preserved byte-for-byte until Phase 5.4.
## Requirements
