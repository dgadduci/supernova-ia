# Capability: metodos-entrega-api

## Purpose

Define the HTTP layer over the existing `MetodosEntrega` model, so consumers (e.g., commerce-to-method associations, client catalog browsing) can list, retrieve, and create delivery methods through the same FastAPI conventions established in earlier API subphases.
