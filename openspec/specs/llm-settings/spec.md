...
- **THEN** the outbound payload contains `model=LLM_MODEL`, `prompt="hola"`, `stream=False`, `think=False`, `format="json"`, `temperature=0`, and the configured numeric options
...
- **THEN** `QueryLlm.request` extracts and parses the substring between the first `{` and last `}`

#### Scenario: Empty response is rejected
- **WHEN** the mocked transport returns an empty body
- **THEN** `QueryLlm.request` raises a clear exception and does not return `None`
