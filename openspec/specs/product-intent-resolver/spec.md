# Capability: product-intent-resolver

## Purpose

Define a single pure function that converts the dict returned by the product recognizer into the four-key output dict the future handler consumes — exposing only the resolver function itself (no recognizer, no handler, no schema, no DB write, no LLM call, no intent contract, no persistence) so other modules can introspect the resolver's contract and reuse it without importing implementation code that performs side effects.
...
When the recognizer returns one or more plausible matches and no confident match, the resolver SHALL collect every plausible match's `producto_presentacion_id` into `candidate_ids` in order, preserve the detected `cantidad` from the first plausible match into `resolved_data["cantidad"]` when present, and leave `resolved_data["producto_presentacion_id"]` empty (the future handler picks from the candidates).
...
- **WHEN** the test calls `resolve_product_intent({"encontrados_posibles": [{"producto_presentacion_id": 10}]})`
...
- **WHEN** the test calls `resolve_product_intent({"encontrados": [{"producto_presentacion_id": 1, "cantidad": 1}]})` (only one key supplied)
