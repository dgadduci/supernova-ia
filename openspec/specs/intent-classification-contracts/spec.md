# Capability: intent-classification-contracts

## Purpose

Define Pydantic contracts for intent classification results — the typed value objects that capture an `IntentName` enum preserving the legacy intent set, a single classified intent with its source message, and an aggregated result carrying one or more classified intents alongside the original message — exposed only as schemas (no LLM calls, prompt construction, HTTP requests, or session/pedido/context mutations) so downstream recognizers, resolvers, processors, and tests can rely on a single source of truth for intent names and classification shapes without importing implementation code.
