# Capability: provider-flow-live-audit

## Purpose

Provide an operator-run, read-only live audit of the integrated provider
flow on the `supernova-ia` Railway service. The audit polls the durable
receipt, processing, LLM timing and outbound rows without mutating
application state so the operator can locate the last persisted boundary
between acceptance, worker processing, LLM finalization and outbound
staging while real Twilio messages flow.

The capability is intentionally a diagnostic projection only. It does
not own transactions, claims, leases, retries, replays or dispatcher
code. Output is safe to share over a chat channel: numeric ids, opaque
receipt fingerprints, closed state/category values, bounded counts and
timestamps only — never bodies, recipients, provider SIDs, prompts,
responses, exception text or credentials.
