# Capability: product-recognizer-baseline-dataset

## Purpose

Provide a version-controlled, version-traceable baseline dataset (`backend/tests/fixtures/product_recognizer_baseline.json` or the project's established equivalent fixture location) that records the current observable behavior of the fuzzy product recognizer across representative exact, ambiguous, restricted-refinement, unknown, alias, multi-word, and known-limitation cases. The dataset exists so that future recognizer implementations can be checked against the same catalog-derived expectations and so that the fuzzy recognizer's known limitations remain explicit rather than implicit.
