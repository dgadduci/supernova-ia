...
Provide an end-to-end integration test for the `quitar_producto` flow that exercises the real recognizer, resolver, processor, dispatcher, handler, response builder, and services against the draft pedido, proving that the new `ORDER_LINE_SELECTION` context type does not corrupt the existing `PRODUCT_SELECTION` flow and that mixed operations across turns stay coherent.
