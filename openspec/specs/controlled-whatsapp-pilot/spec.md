# controlled-whatsapp-pilot Specification

## Purpose
TBD - created by archiving change run-controlled-whatsapp-pilot-7-1. Update Purpose after archive.
## Requirements
...
durable deferred inbound work item, explicit bounded inbound processor, durable
outbox, explicit bounded dispatcher and delivery callback path without
introducing an automatic dispatcher, worker, scheduler, direct database
mutation or alternate message pipeline. Before a live inbound case, the
deployed Alembic revision SHALL be at head.
...
- **THEN** the existing signed webhook persists deferred work, one explicit
  bounded inbound-processing pass owns business processing, and any customer
  response is dispatched only by the existing explicit bounded dispatcher

#### Scenario: Deferred inbound schema is unavailable

- **WHEN** the deployed Alembic revision is not at head or the deferred work
  item table is unavailable
- **THEN** the pilot stops before another live inbound case, and migration plus
  deployment verification require separate explicit operator approval
