# Capability: ollama-embedding-client

## Purpose

Provide a synchronous, provider-focused embedding client that generates validated single and batched embeddings through the local Ollama `/api/embed` endpoint, with embedding-specific configuration independent from the Qwen text-generation settings, deterministic ordering, bounded batches, strict response validation, and domain-specific failure types so later semantic-search phases can consume vectors without coupling to persistence or catalog concerns.
