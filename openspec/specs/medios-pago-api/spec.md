# Capability: medios-pago-api

## Purpose

Define the HTTP layer over the existing `MediosPago` model, so future subphases that surface payment-method references on other resources (e.g., `Comercio` joins, payment-method selection on incoming orders) can rely on a catalog reachable through the same FastAPI conventions established in Subphases 2.1 and 2.2.
