# Capability: product-presentation-embedding-indexing
...
#### Scenario: Subphase 4.8 sync uses the same projection with product filter

- **WHEN** the `CatalogEmbeddingSynchronizationService.synchronize_producto(7)` runs after a product name change
- **THEN** the existing projection is invoked with `id_producto=7` through the existing public surface
- **AND** the resulting embeddings cover the same scope the manual reindex would have covered for the same `id_producto`
...
#### Scenario: Subphase 4.8 sync reuses the indexer's unchanged / failed / stale behavior

- **WHEN** the `CatalogEmbeddingSynchronizationService` invokes the indexer after a product name change
- **THEN** the indexer reports `unchanged` for documents whose `content_hash` matches the stored row and the five unchanged conditions hold
- **AND** the indexer reports `updated` for documents whose `content_hash` differs
- **AND** the indexer reports `failed` for documents whose batch failed
- **AND** the Subphase 4.8 service does NOT short-circuit or rewrite any of these outcomes
...
The system SHALL provide a `ProductoPresentacionEmbeddingSeeder` that wraps the indexer and returns a `SeedingResult` carrying `(created, updated, unchanged, stale, inactive, failed)` aggregate counts plus a per-presentation `SeedingOutcome` list. The seeder SHALL NOT call `session.commit()`, `session.rollback()`, or `session.close()`. The CLI is the only owner of those calls. The seeder's `dry_run=True` mode is strictly read-only: it projects the catalog, classifies every document through the same hash-based idempotency path, classifies the planned `stale` and `inactive` outcomes, and returns the same `SeedingResult` shape WITHOUT calling `mark_stale`, `mark_inactive`, `embed_documents`, `flush`, or `commit`, and WITHOUT inserting or updating rows. The Subphase 4.8 `CatalogEmbeddingSynchronizationService` SHALL invoke this seeder with `dry_run=False`; the seeder's `SeedingResult` is consumed by the Subphase 4.8 service to build the `EmbeddingSynchronizationResult`. The seeder SHALL NOT be modified to accept a new `EmbeddingSynchronizationResult` constructor argument.
...
#### Scenario: Subphase 4.8 sync invokes the seeder without dry-run

- **WHEN** the `CatalogEmbeddingSynchronizationService.synchronize_producto_presentacion(31)` runs
- **THEN** the seeder is invoked with `dry_run=False` and the matching `id_producto_presentacion=31` filter
- **AND** the seeder's `SeedingResult` aggregates flow into the Subphase 4.8 `EmbeddingSynchronizationResult` counters without modification
...
#### Scenario: Subphase 4.8 sync is not reachable through the admin router

- **WHEN** the admin router module is inspected
- **THEN** it does NOT import `CatalogEmbeddingSynchronizationService` or any Subphase 4.8 service symbol
- **AND** the route handlers do NOT invoke the sync service

#### Scenario: Subphase 4.8 sync reuses the same seeder counters

- **WHEN** `CatalogEmbeddingSynchronizationService.synchronize_producto(7)` runs after a product name change
- **THEN** the `EmbeddingSynchronizationResult.created` counter equals the sum of the per-presentation `SeedingResult.created` values the seeder returns
- **AND** the same equality holds for `updated`, `unchanged`, `stale`, `inactive`, and `failed`
- **AND** the Subphase 4.8 service does NOT transform or reclassify the seeder's per-document outcomes
