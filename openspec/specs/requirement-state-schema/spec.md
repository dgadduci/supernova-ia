# Capability: requirement-state-schema

## Purpose

Define a Pydantic schema that the future recognizer and handler adapters will use to track and validate the state of a single contract requirement at runtime, exposing only the schema itself — no registry, recognizer, handler, or model — so other modules can introspect the typed shape of a requirement's state without importing implementation code.
