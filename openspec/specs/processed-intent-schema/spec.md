# Capability: processed-intent-schema

## Purpose

Define a Pydantic schema that the future recognizer and handler adapters use to represent one processed intent at runtime — the typed value object that captures the intent name, the source text it was extracted from, the resolution status, the recognizer and handler names, the resolved slot data, the per-requirement states, and any candidate ids for ambiguous cases — exposing only the schema itself (no registry, recognizer, handler, or model) so other modules can introspect the typed shape of a processed intent without importing implementation code.
