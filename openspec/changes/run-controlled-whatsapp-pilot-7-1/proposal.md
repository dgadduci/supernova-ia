...
`Twilio WhatsApp webhook -> ProviderInboundMessageCoordinator -> existing
incoming-message orchestration -> durable outbound provider-message outbox ->
explicit backend.cli.run_outbound_dispatch pass -> Twilio delivery callback`.
...
- Require explicit, bounded outbound-dispatch passes and inspect safe delivery
  outcomes.
...
Existing inbound coordinator, outbox dispatcher and callback transaction
boundaries remain unchanged. Evidence may contain timestamps,
...
release/revision/health evidence; the existing safe Ollama contract probe; a
CLI happy path and ambiguity path with `--debug-flow`; routing verification;
one controlled WhatsApp inbound case; one explicitly invoked bounded outbound
dispatch; and a signed delivery callback outcome. Exact commands are in
