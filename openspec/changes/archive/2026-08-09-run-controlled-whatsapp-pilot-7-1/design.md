...
flowchart LR
  ...
  TW --> WORK["Durable inbound work item"]
  WORK --> IN["Explicit bounded inbound processor"]
...
- Inbound business processing is likewise performed only by an explicit
  `backend.cli.run_inbound_processing --max-items-per-pass 1` pass. The
  webhook accepts and persists the work item; it does not itself create an
  outbound row.
- Before a live inbound case, `alembic current` SHALL equal the deployed head.
  A missing deferred-work-item table is a deployment gate failure: stop the
  pilot, apply the approved migration, redeploy/verify, and do not treat an
  earlier non-persisted inbound delivery as a successful case.
...
| --- | --- | --- |
| Deployment | release ID, Alembic revision at head, `/health` status | Expected revision and 200 health response |
...
| WhatsApp inbound | safe receipt/work-item IDs and processing result | Webhook persists one work item; one bounded inbound pass processes it once |
...
After a `ready` result and Alembic-head confirmation, send one approved inbound
case, run exactly one inbound pass with `--max-items-per-pass 1`, then exactly
one manual dispatcher pass with `--max-attempts-per-pass 1`. Do not paste
secrets, E.164 values, raw request bodies or generated content into the
terminal report.
