...
La instrumentación se desplegó sólo en `calibracion` y produjo tres reportes persistentes elegibles, sin `--diagnose`, Twilio, mensajes reales ni acceso a producción. Los p95 totales fueron `451.1 ms`, `439.9 ms` y `391.9 ms`, todos bajo el presupuesto congelado de `500 ms`.

Los tres reportes seleccionaron la misma política: `fuzzy_weight=0.5`, `vector_weight=0.5`, `unique_threshold=0.65`, `ambiguous_threshold=0.4`, `minimum_score_gap=0.05` y `vector_top_k=3`. La evidencia permitió conservar el presupuesto sin elevarlo y sirvió como prerrequisito de los changes de promoción posteriores, que tienen su propia autoridad y evidencia.
