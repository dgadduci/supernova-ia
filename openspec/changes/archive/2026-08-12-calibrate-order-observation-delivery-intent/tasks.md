...
- [x] 2.1 Bump `PROMPT_TEMPLATE_VERSION` to
...
- [x] 2.2 Append the four contrastive `Mensaje:` / `Salida:` /
...
- [x] 2.3 Bump `CORPUS_VERSION` to `intent-corpus/v1.2.0` and append
...
- [x] 2.4 Add focused regression tests in
...
- [x] 2.5 Update any pre-existing version-literal pin in the four
...
- [x] 3.1 User ran locally and shared complete output of:
  ...
  Verified results (initial phase pass and post-correction re-run
  both produced the same green output):
  - pytest: `75 passed, 164 subtests passed`
  - Ruff: clean
  - compileall: clean
  - `openspec validate calibrate-order-observation-delivery-intent --strict`: valid

- [x] 3.2 Codex reviewed the diff against scope, privacy,
  ...
  and approved the address-priority correction (rule 8 now explicitly
  gives `set_direccion_entrega` precedence over `set_observacion_pedido`
  when the message contains a domicilio or dirección concreta, and the
  `F-REG-DIRECCION_ENTREGA-TILCARA_2020` regression fixture is pinned).
