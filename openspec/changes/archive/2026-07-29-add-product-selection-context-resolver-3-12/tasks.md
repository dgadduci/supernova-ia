...
- [x] 1.1 Create `backend/intents/context/product_selection_context_resolver.py` exporting one function and a `__all__`:
  ...
  - The function:
    ...
    7. Otherwise builds a new `ProcessedIntent` with:
       ...
       - `source_text=active_intent.source_text`
        - `status="ready"` if every requirement is `completed`, else `"pending_resolution"`
...
- [x] 2.1 Add unit tests to `backend/tests/api_smoke.py` that:
  - import `resolve_product_selection`, `ProcessedIntent`, `RequirementState`;
  - patch `backend.intents.context.product_selection_context_resolver.detectar_productos` with a mock that returns a controllable dict;
  - use a SQLAlchemy in-memory SQLite database (e.g. `engine = create_engine("sqlite:///:memory:")`, `Base.metadata.create_all(engine)`, `sessionmaker(bind=engine)`) to set up a real DB; insert sample `ProductoPresentacion` rows joined to `Producto`, `Presentacion`, `CategoriaProducto`; do NOT use the project's `supernova_test` DB to keep the test isolated;
  - assert the function issues a SQLAlchemy query with `.where(id.in_(...))` filtered to `active_intent.candidate_ids` (verified by inspecting the query or by inserting only matching rows and asserting they appear, plus an extra non-matching row that does NOT appear in the result);
  - assert unique selection by presentation: the mock returns a single `encontrados` item with `producto_presentacion_id` in `candidate_ids`; the function returns a new `ProcessedIntent` with `producto_presentacion_id` set, the requirement marked `completed`, `candidate_ids=[]`, and the original `resolved_data` (including `cantidad`) preserved;
  - assert the function calls `detectar_productos` exactly once with the user `message` and the built 12-field catalog;
  - assert original `cantidad` is preserved;
  - assert ambiguous result (2+ items in `encontrados`) returns `active_intent` unchanged;
  - assert unavailable result (only `encontrados_no_disponibles` populated) returns `active_intent` unchanged;
  - assert unknown result (no `encontrados`) returns `active_intent` unchanged;
  - assert selected ID outside original `candidate_ids` returns `active_intent` unchanged;
   - assert fully resolved intent has `status="ready"` when every requirement is completed; assert status is explicitly `"pending_resolution"` when another requirement is still pending;

  - assert the function does NOT call `db.commit` (verified by mock);
  - assert the function does NOT modify the `active_intent` instance (the same object is returned when no resolution happens);
  - assert the module's `__all__` is exactly `{"resolve_product_selection"}`;
  - assert `backend/intents/context/` contains exactly `__init__.py`, `context_type_resolver.py`, `pending_context_service.py`, and `product_selection_context_resolver.py`.
- [x] 2.2 Add a separate test entry to `backend/tests/api_smoke.py` that is a real integration test with the actual recognizer:
  - uses a SQLAlchemy in-memory SQLite database (e.g. `engine = create_engine("sqlite:///:memory:")`, `Base.metadata.create_all(engine)`, `sessionmaker(bind=engine)`) to set up a real DB; insert two `ProductoPresentacion` rows for the same product (e.g. `Pizza Mozzarella`) with different presentations ("chica" and "grande") plus a third `ProductoPresentacion` row for a different product that should NOT be selected; do NOT use the project's `supernova_test` DB;
