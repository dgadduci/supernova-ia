# Capability: railway-hybrid-recognition-calibration-policy

## Purpose

Provide a controlled, read-only Railway calibration gate for the frozen product
recognition dataset, so a later explicitly approved authoritative-hybrid
activation can rely on an eligible, persistent and verifiable policy artifact.
