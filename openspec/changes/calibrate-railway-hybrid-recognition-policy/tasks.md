...
- [ ] 1.1 Confirmar que esta etapa sólo calibra y revisa: no activa
  `hybrid_authoritative`, no cambia configuración y no ejecuta Twilio.
- [ ] 1.2 Confirmar el destino Railway controlado y de sólo lectura para las
  referencias dinámicas del dataset congelado; no usar catálogos, clientes ni
  tráfico real.
- [ ] 1.3 Confirmar el mecanismo persistente Railway y su ruta real sólo tras
...
- [ ] 2.1 Verificar que `PRODUCT_RECOGNIZER_MODE=shadow` y que los gates
  Railway de embedding ya son sanos, sin modificar Tailscale/Ollama/UFW.
- [ ] 2.2 Ejecutar el CLI existente una vez con
...
- [ ] 2.3 Revisar `eligibility.status` como veredicto autoritativo. Si no es
  `eligible`, detenerse y mantener shadow.
- [ ] 2.4 Si es `eligible`, verificar persistencia, hash y lectura del reporte
...
- [ ] 3.2 Proponer un cambio separado para cualquier configuración controlada
